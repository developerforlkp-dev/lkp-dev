...
export function BookingSystem({ listing, type = "experience", selectedAddOns = [], triggerLabel = "Reserve Now", reserveLabel = "Reserve Experience", onUpdateAddonQuantity }) {
  ...
  const handleReserve = async () => {
      ...
      try {
        ...
        const bookingData = {
          ...
          receipt: (() => {
            ...
            return eventReceipt; // Don't forget to return the array at the end of the IIFE
          })(),
            {
          title: "Total",
    ...
    if (
      ...
      // 1. Clear invalid/expired token
      localStorage.removeItem("jwtToken");
      localStorage.removeItem("userInfo");

      // 2. Save the pending booking state so the user can continue after login
      const listingIdToSave = listing?.listingId || listing?.id || listing?.eventId || listing?.stayId;
      if (listingIdToSave) {
        const stateToStore = {
          listingId: String(listingIdToSave),
          ...
          privateBooking,
          selectedAddOns: selectedAddOns.map(a => a?.addon?.addonId || a?.addonId || a?.id),
          addOnQuantities: addOnQuantities, // Kept from current
        };
        try {
          localStorage.setItem("frontendPendingBookingState", JSON.stringify(stateToStore));
        } catch (err) { }
      ...
      // 3. Trigger login prompt & safely turn off loading state
      setShowLoginPrompt(true);
      if (isMountedRef.current) setBookingLoading(false); // Added from incoming
      return; // Added from incoming to cleanly exit the function
    } else {
      // 4. Handle all other non-auth errors
  ...
  if (
    ...
    if (listingIdToSave) {
      const stateToStore = {
        ...
        privateBooking,
        selectedAddOns: selectedAddOns.map(a => a?.addon?.addonId || a?.addonId || a?.id),
        // Preserved your exact custom IIFE object builder from current code
        addOnQuantities: (() => {
          const quantities = {};
          selectedAddOns.forEach(item => {
            const addon = item.addon || item;
    ...
    // 3. Trigger login prompt & safely turn off UI loading flags
    setShowLoginPrompt(true);
    if (isMountedRef.current) setBookingLoading(false); // Brought over from incoming
    return; // Brought over from incoming to prevent falling through to subsequent logic
  } else {
    // 4. Handle all non-auth errors
    alert(errorMsg || "Booking failed. Please try again.");
  }
} finally {
  if (isMountedRef.current) setBookingLoading(false);
