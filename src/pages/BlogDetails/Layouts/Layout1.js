...
// Layout 1 — EDITORIAL TIMELINE STYLE
// (Originally Layout 4 AsymmetricMosaic from blogpage)
export default function Layout1({ post }) {
  ...
  return (
    <div className="min-h-screen bg-white text-[#001F3F] pb-20">
        ...
        <div className="absolute bottom-0 left-0 w-full translate-y-[30%] z-20 px-6 lg:px-12 overflow-x-auto">
          <div className="flex justify-center gap-4 min-w-max pb-4">
            {cardImages.map((img, i) => (
              ...
              >
                <img src={img} alt={`Highlight ${i+1}`} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent" />
                <div className="absolute bottom-4 left-4 right-4">
                  <p className="text-white text-xs font-bold uppercase tracking-wider line-clamp-2">
                    {post.tags && post.tags[i] ? post.tags[i] : `Highlight 0${i+1}`}
                  </p>
                </div>
              </motion.div>
            ))}
      ...
      {/* ── Summary / Intro ── */}
      <div className="w-full max-w-[1000px] mx-auto px-6 mb-24 text-center mt-12">
                  ...
                  <div className={`w-full lg:w-1/2 flex ${isEven ? 'justify-start' : 'justify-end'} ${isEven ? 'lg:pl-16' : 'lg:pr-16'}`}>
                    {section.image ? (
                      <div className="w-full max-w-[360px] h-[280px] relative rounded-2xl overflow-hidden shadow-lg border border-gray-100 group">
                        <img src={section.image} alt={section.heading} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                      </div>
                    ) : (
